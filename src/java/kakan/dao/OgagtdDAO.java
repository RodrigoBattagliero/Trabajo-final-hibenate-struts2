...
public class OgagtdDAO {
    ...
    private final String SELECT_DESIGNACIONES = 
                        "SELECT DISTINCT id,nro_norma as nro_resolucion, nombre_categoria,fec_norma, fecha_inicio, fecha_fin "
                        + "FROM kakan.v_ogagtd "
                        + "WHERE "
    ...
    private final String SELECT_FROM_DESIGNACION = 
                        "SELECT unidad_academica,nombre_unidad_academica, carrera, nombre_carrera,plan,materia,nombre_materia,comision "
                        + "FROM kakan.v_ogagtd "
                        + "WHERE  id=?";
   ...
   public List<VOgagtd> selectFromDesignacion(int idDesignacion){
       PreparedStatement ps;
        ResultSet res;
        ArrayList vOgagtdList = new ArrayList();
        try {
            ps = conex.getCnn().prepareStatement(SELECT_FROM_DESIGNACION);
            ps.setInt(1, idDesignacion);
            res = ps.executeQuery();
            while(res.next()){
                vOgagtdList.add(new VOgagtd(
                        res.getInt("comision"),
